...
## Visual Parity Workflow (Required for Styling Tasks)

When matching an existing site design, follow this workflow:

1. Use `http://localhost:3000` as the primary debugging target.
2. Compare against the source site for visual parity (spacing, sizing, colors, responsive behavior).
3. Inspect and style the local block DOM only; do not try to clone the source site's DOM structure.
4. Prioritize matching layout behavior first (full-bleed/background coverage, constrained content width, alignment), then typography/detail tuning.
5. Validate with screenshots at relevant breakpoints before finishing.
